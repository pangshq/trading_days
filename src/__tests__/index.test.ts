...
import { isTradingDay, getTradingDayDetail } from '../index';

describe('isTradingDay', () => {
  // Weekends
  it('returns false for Saturday', () => {
    expect(isTradingDay('2025-05-10')).toBe(false); // Saturday
  });

  it('returns false for Sunday', () => {
    expect(isTradingDay('2025-05-11')).toBe(false); // Sunday
  });

  // Normal workdays
  it('returns true for normal weekday', () => {
    expect(isTradingDay('2025-05-12')).toBe(true); // Monday
  });

  // Holidays
  it('returns false for National Day', () => {
    expect(isTradingDay('2025-10-01')).toBe(false);
  });

  it('returns false for Spring Festival', () => {
    expect(isTradingDay('2025-01-29')).toBe(false);
  });
  ...
  it('accepts timestamp', () => {
    ...
    expect(isTradingDay(ts)).toBe(false);
  });
...
describe('getTradingDayDetail', () => {
  it('returns correct structure for workday', () => {
    const detail = getTradingDayDetail('2025-05-12');
  ...
  it('returns correct structure for holiday', () => {
    const detail = getTradingDayDetail('2025-10-01');
  ...
  it('defaults to today', () => {
    const detail = getTradingDayDetail();
